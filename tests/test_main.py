...
from app.main import AnnotationResponse, app, authenticate
...
async def test_export():
    async with AsyncClient(app=app, base_url="http://test") as test_client:
        ...
        response = await test_client.post(
            "/export/3398820/1192052", headers={"Authorization": f"Basic {auth_header}"}
        )
...
def test_authenticate_success():
    response = client.get("/secure-endpoint", auth=(USERNAME, PASSWORD))
    assert response.status_code == 200
    assert response.json() == {"success": True}


def test_authenticate_failure():
    response = client.get("/secure-endpoint", auth=("wrongUser", "wrongPassword"))
