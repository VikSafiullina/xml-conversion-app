...
from app.config import LOGGING_LEVEL, PASSWORD, USERNAME
...
async def export_data(
    annotation_id: int = Path(
        ..., title="Annotation ID", description="The ID of the annotation", ge=1
    ),
    queue_id: int = Path(
        ..., title="Queue ID", description="The ID of the queue", ge=1
    ),
    ...
    try:
        ...
        post_data = {"annotationId": annotation_id, "content": encoded_xml}
        ...
        raise HTTPException(
            status_code=e.response.status_code, detail="Failed to fetch annotation data"
        )
