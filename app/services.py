import httpx
import redis
from app.config import ROSSUM_API_KEY, ROSSUM_BASE_URL, DUMMY_ENDPOINT, CACHE_DURATION, REDIS_HOST, REDIS_PORT
import logging
...
async def fetch_annotation_data(annotation_id: str, queue_id: str):
    ...
    if cached_data:
        logger.info("Returning cached data for annotation %s in queue %s", annotation_id, queue_id)
    ...
    async with httpx.AsyncClient() as client:
        ...
        logger.info("Fetched and cached new data for annotation %s in queue %s", annotation_id, queue_id)
